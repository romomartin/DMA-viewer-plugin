...
class Plugin implements PluginI<Message> {
  ...
  onMessage(sdk: SDK, message: Message) {
    if(message.event === events.changeDMAcolor) {
      const changedDmaIndex = this.dmas.findIndex((dma)=>dma.id === message.dma.id)
...
const changeDmaLayer = (sdk:SDK, overlay:OverlayLayer<"GeoJsonLayer">[], dma:DMA):OverlayLayer<"GeoJsonLayer">[] => {
  const dmaPipes = sdk.network
  .getZone(dma.id)
  ?.getPipes((pipe) => pipe.group === PipeGroups.Main);
